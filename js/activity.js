...
class Activity {
    ...
    constructor() {
        ...
        this._displayHelpfulWheel = (event) => {
            ...
            const selectedBlocksCount = this.blocks.selectedBlocks.filter((block) => !block.trash)
                .length;
         ...
         */
        /**
         * Initialize search functionality for blocks using autocomplete
         */
        this.initBlockSearch = () => {
            ...
            const searchInputEl = $j("#search");
            if (!searchInputEl.length) return;

            searchInputEl.autocomplete({
                source: self.searchSuggestions || [],
                select: (event, ui) => {
                    event.preventDefault();

                    self.searchWidget.value = ui.item.label;
                    self.searchWidget.idInput_custom = ui.item.value;
                    self.searchWidget.protoblk = ui.item.specialDict;

                    self.handleBlockSelection();

                    if (event.keyCode === 13) {
                        self.searchWidget.style.visibility = "visible";
                    }
                },
                focus: (event) => event.preventDefault()
            });

            searchInputEl.autocomplete("instance")._renderItem = (ul, item) => {
                return $j("<li></li>")
                    .data("item.autocomplete", item)
                    .append(`<img src="${item.artwork}" height="20px"> <a>${item.label}</a>`)
                    .appendTo(
                        ul.css({
                            "z-index": 9999,
                            "max-height": "200px",
